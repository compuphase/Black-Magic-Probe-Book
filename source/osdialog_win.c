...
char* osdialog_file(osdialog_file_action action, const char* title, const char* dir, const char* filename, osdialog_filters* filters) {
	if (action == OSDIALOG_OPEN_DIR) {
		...
		if (filename) {
			...
			wcsncpy(strFile, filenameW, MAX_PATH);
		...
		// caption
		wchar_t strTitle[MAX_PATH] = L"";
		if (title) {
			wchar_t* titleW = utf8_to_wchar(title);
			wcsncpy(strTitle, titleW, MAX_PATH);
			OSDIALOG_FREE(titleW);
