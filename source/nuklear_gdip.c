...
struct GdipFont
{
    ...
    GpFont* handle;
    int voffset;    /* vertical offset in pixels, for improved alignment */
};
...
GdipFont*
nk_gdipfont_create(const char *name, float size, int style)
{
    ...
    if (!font)
        return NULL;
    if (GdipCreateFontFamilyFromName(wname, NULL, &family)) {
        free(font);
        return NULL;
    }
    if (GdipCreateFont(family, size, style, UnitPixel, &font->handle)) {
        free(font);
        return NULL;
    }
...
GdipFont*
nk_gdipfont_create_from_collection(float size, int style)
{
    ...
    if (!font)
        return NULL;
    if( GdipGetFontCollectionFamilyCount(nk_gdip_getCurFontCollection(), &count) ) {
        free(font);
        return NULL;
    }
    ...
    if( GdipGetFontCollectionFamilyList(nk_gdip_getCurFontCollection(), count, families, &count) ) {
        free(families);
        free(font);
        return NULL;
    }
    ...
    if( GdipCreateFont(families[count-1], size, style, UnitPixel, &font->handle) ) {
        free(families);
        free(font);
        return NULL;
    }
...
GdipFont*
nk_gdipfont_create_from_memory(const void* membuf, int membufSize, float size, int style)
{
    ...
    return nk_gdipfont_create_from_collection(size, style);
}

GdipFont*
nk_gdipfont_create_from_file(const WCHAR* filename, float size, int style)
{
    if( !nk_gdip_getCurFontCollection() )
        if( GdipNewPrivateFontCollection(&gdip.fontCollection[gdip.curFontCollection]) ) return NULL;
...
void nk_gdipfont_set_voffset(GdipFont *font, int voffset)
{
    if (font)
        font->voffset = voffset;
}
...
nk_gdip_handle_event(HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_SYSKEYUP:
    {
        ...
        int shift = GetKeyState(VK_SHIFT) & (1 << 15);
        int alt = GetKeyState(VK_MENU) & (1 << 15);
        ...
        case VK_ESCAPE:
            if (!ctrl && !shift && !alt) {
                nk_input_key(&gdip.ctx, NK_KEY_ESCAPE, down);
                return 1;
            }
            break;
        ...
        case VK_HOME:
            if (ctrl) {
                nk_input_key(&gdip.ctx, NK_KEY_SCROLL_TOP, down);
                return 1;
            } else if (!shift && !alt) {
        ...
        case VK_END:
            if (ctrl) {
                nk_input_key(&gdip.ctx, NK_KEY_SCROLL_BOTTOM, down);
                return 1;
            } else if (!shift && !alt) {
        ...
        case VK_F12:
            if (shift) {
                nk_input_key(&gdip.ctx, (wparam - VK_F1) + NK_KEY_SHIFT_F1, down);
                return 1;
            } else if (ctrl) {
                nk_input_key(&gdip.ctx, (wparam - VK_F1) + NK_KEY_CTRL_F1, down);
                return 1;
            } else if (!alt) {
                nk_input_key(&gdip.ctx, (wparam - VK_F1) + NK_KEY_F1, down);
                return 1;
            }
            break;
        ...
        case 'F':
            if (ctrl) {
                nk_input_key(&gdip.ctx, NK_KEY_FIND, down);
                return 1;
            }
            break;
        ...
        case 'R':
            if (ctrl) {
                nk_input_key(&gdip.ctx, NK_KEY_REFRESH, down);
                return 1;
            }
            break;
